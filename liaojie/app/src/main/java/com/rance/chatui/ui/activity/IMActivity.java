...
public class IMActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       setContentView(R.layout.activity_main);
        ...
        String currentFriendName = intent.getStringExtra("friendName");
        String currentFriendUsername = intent.getStringExtra("friendUsername");
        ChatContext.getContext().setCurrentChatFriendName(currentFriendUsername);
        ChatContext.getContext().addDialog(currentFriendUsername,this);
    ...
    protected void onResume() {
        super.onResume();
        List<MessageOuterClass.Message> unReadMsg = ChatContext.getContext().getUnReadMsg("");
...
//            chatContextMenu.setAnimationStyle();
    ...
    private void LoadData() {
        ...
        messageInfos = ChatHistory.getInstance().getChatHistory(friendUsername);
    ...
    public void MessageEventBus(final MessageInfo messageInfo) {
        ...
        String friendUsername = getIntent().getStringExtra("friendUsername");
        //messageInfos = ChatHistory.getChatHistory().getChatHistory(friendUsername);
       // messageInfos.add(messageInfo);
        ChatHistory.getInstance().addMessege(friendUsername,messageInfo);
        int size = ChatHistory.getInstance().getChatHistory(friendUsername).size();
        chatList.scrollToPosition(chatAdapter.getItemCount() - 1);
        chatAdapter.notifyItemInserted(size-1 );

         //发送消息到服务端
        chatWithFriend(ChatContext.getContext().getUsername(),friendUsername,messageInfo.getContent());
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void recieverMsg(final MessageOuterClass.Message message) {

        new Handler().postDelayed(new Runnable() {
            public void run() {
                MessageInfo messageInfo = new MessageInfo();
                messageInfo.setContent(message.getMsg());
    ...
    public void chatWithFriend(String username,String friendUsername,String msg){
        ChannelHandlerContext ctx = ChatContext.getContext().getChannelHandlerContext();
        MsgSender<MessageOuterClass.Message> sender = new MsgSender<>(ctx);
        MessageOuterClass.Message message = MessageOuterClass.Message.newBuilder()
                .setFrom(username).setTo(friendUsername).setMsg(msg).build();
        System.out.println("from:"+username+",to:"+friendUsername+",msg:"+msg);
        sender.send(message);
    }
}
